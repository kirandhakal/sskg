                            ...
                            <Card className="h-full flex flex-col group border-border shadow-xl hover:shadow-2xl hover:shadow-brand/5 transition-all duration-500 rounded-3xl overflow-hidden bg-card">
                                ...
                                <CardContent className="p-8 flex-grow">
                                    <div className="space-y-4">
                                        <h3 className="text-2xl font-bold text-foreground group-hover:text-brand transition-colors">{room.name}</h3>
                                        <p className="text-muted-foreground line-clamp-2 leading-relaxed">
                                            {room.description}
                                        </p>
                                        <div className="flex flex-wrap gap-2 pt-2">
                                            {room.amenities.slice(0, 3).map((amenity) => (
                                                <span key={amenity} className="text-xs font-bold uppercase tracking-wider bg-brand/10 text-brand px-3 py-1 rounded-lg">
                                                    {amenity}
                                                </span>
                                            ))}
                                        </div>
